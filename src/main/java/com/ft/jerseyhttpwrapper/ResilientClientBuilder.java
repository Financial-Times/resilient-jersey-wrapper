...
public class ResilientClientBuilder {
    ...
    private HostAndPortIpResolver hostAndPortIpResolver = new HostAndPortIpResolver(new HostToIpMapper());
    ...
    public ResilientClientBuilder using(List<SimpleEndpointConfiguration> primaryNodes, List<SimpleEndpointConfiguration> secondaryNodes) {
    ...
    public ResilientClient build() {
        ...
        if(primaryProvider ==null) {
            ...
            primaryProvider = new HostAndPortProviderBuilder(hostAndPortIpResolver)
        ...
        if(secondaryProvider==null && secondaryNodes!=null && !secondaryNodes.isEmpty()) {
            secondaryProvider = new HostAndPortProviderBuilder(hostAndPortIpResolver)
					.withStrategy(resilienceStrategy)
        ...
        final ResilientClient client = new ResilientClient(
				shortName,
        ...
        ExecutorService threadPool = environment.createExecutorService(shortName,
                jerseyClientConfig.getMinThreads(),
                jerseyClientConfig.getMaxThreads()
		);
    ...
    private String getShortName(boolean useAdminPorts) {
        ...
        String shortName = configuration.getShortName().or(String.format("%s-%d", configuration.getHost(), getPort()));
        if (useAdminPorts) {
