...
public class HostAndPortProviderBuilder {
	...
	public HostAndPortProviderBuilder withSimpleEndpointConfiguration(List<SimpleEndpointConfiguration> endpoints) {
		this.endpoints = endpoints;
		return this;
	}

	public HostAndPortProvider build() {
		List<HostAndPort> nodes = new SimpleEndpointConfigurationToHostPortConverter(useAdminPorts).convertAll(endpoints);
		...
		switch (strategy) {
            ...
            case DYNAMIC_RANDOM_IP_STRATEGY:
                if(nodes!=null) {
                    throw new IllegalArgumentException("You should not provide default routes for use with a dynamic strategy");
                }
