...
public class ResilientJerseyClientWithSimpleFailoverStrategyTest {
  ...
  @ClassRule public static WireMockClassRule wmClassRule4 = new WireMockClassRule(0);

  @Rule public WireMockClassRule instanceRule4 = wmClassRule4;
  ...
  public void setup() {
    resilientClient =
        ResilientClientBuilder.inTesting(HostAndPort.fromParts("localhost", instanceRule1.port()))
            .withPrimary(
                new SimpleHostAndPortProvider(
  ...
  public void shouldFailoverAcrossPairsOfHosts() {
    ...
    stubForWireMockClassRule(
        wmClassRule1,
        get(urlMatching(".*")).willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
    stubForWireMockClassRule(
        wmClassRule2,
        get(urlMatching(".*")).willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
    stubForWireMockClassRule(
        ...
        get(urlMatching(".*")).willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
    stubForWireMockClassRule(
        wmClassRule4, get(urlMatching(".*")).willReturn(aResponse().withStatus(200)));
  ...
  public void shouldFailoverAcrossPairsOfHostsWithPUTAndPayload() {
    ...
    stubForWireMockClassRule(
        wmClassRule1,
        put(urlMatching(".*")).willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
    stubForWireMockClassRule(
        wmClassRule2,
        put(urlMatching(".*")).willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
    stubForWireMockClassRule(
        ...
        put(urlMatching(".*")).willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
    stubForWireMockClassRule(
        wmClassRule4, put(urlMatching(".*")).willReturn(aResponse().withStatus(200)));
    ...
    resilientClient.resource(uri).header("Content-Type", "text/plain; charset=UTF-8").put(PAYLOAD);

    // assertThat(clientResponse.getStatus(), is(200));

    verifyBehaviourForClassRule(
        wmClassRule1, putRequestedFor(urlMatching("/testing")).withRequestBody(equalTo(PAYLOAD)));
    verifyBehaviourForClassRule(
        wmClassRule2, putRequestedFor(urlMatching("/testing")).withRequestBody(equalTo(PAYLOAD)));
    verifyBehaviourForClassRule(
        wmClassRule3, putRequestedFor(urlMatching("/testing")).withRequestBody(equalTo(PAYLOAD)));
    verifyBehaviourForClassRule(
        wmClassRule4, putRequestedFor(urlMatching("/testing")).withRequestBody(equalTo(PAYLOAD)));
  }
  ...
  private void assertFailoverAndReportingOnStatusCode(int failoverStatus) {
    ...
    stubForWireMockClassRule(
        wmClassRule1, get(urlMatching(".*")).willReturn(aResponse().withStatus(failoverStatus)));
    stubForWireMockClassRule(
        wmClassRule2, get(urlMatching(".*")).willReturn(aResponse().withStatus(failoverStatus)));
    stubForWireMockClassRule(
        wmClassRule3, get(urlMatching(".*")).willReturn(aResponse().withStatus(failoverStatus)));
    stubForWireMockClassRule(
        wmClassRule4, get(urlMatching(".*")).willReturn(aResponse().withStatus(failoverStatus)));
  ...
  public void shouldReportServerErrorRatherThanNetworkErrorIfOutcomeIsMixed() {
    ...
    stubForWireMockClassRule(
        ...
        get(urlMatching(".*")).willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
    stubForWireMockClassRule(
        wmClassRule2, get(urlMatching(".*")).willReturn(aResponse().withStatus(500)));
    stubForWireMockClassRule(
        wmClassRule3, get(urlMatching(".*")).willReturn(aResponse().withStatus(500)));
    stubForWireMockClassRule(
        wmClassRule4,
        get(urlMatching(".*")).willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
  ...
  public void shouldNotFailoverToSecondListIfFirstListIsUp() {
    ...
    stubForWireMockClassRule(
        wmClassRule1,
        post(urlMatching(".*")).willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
    stubForWireMockClassRule(
        wmClassRule2, post(urlMatching(".*")).willReturn(aResponse().withStatus(201)));
    stubForWireMockClassRule(
        wmClassRule3, post(urlMatching(".*")).willReturn(aResponse().withStatus(500)));
    stubForWireMockClassRule(
        wmClassRule4, post(urlMatching(".*")).willReturn(aResponse().withStatus(500)));

    URI uri = UriBuilder.fromPath("testing").scheme("http").host("localhost").build();
  ...
  public void shouldNotExhaustConnectionPoolForSlow503s() throws ExecutionException {
    ...
    stubForWireMockClassRule(
        wmClassRule1, get(urlMatching(testUrl)).willReturn(aSlow503WithBody(responseSpeed)));
    stubForWireMockClassRule(
        wmClassRule2, get(urlMatching(testUrl)).willReturn(aSlow503WithBody(responseSpeed)));
    stubForWireMockClassRule(
        wmClassRule3, get(urlMatching(testUrl)).willReturn(aSlow503WithBody(responseSpeed)));
    stubForWireMockClassRule(
        wmClassRule4, get(urlMatching(testUrl)).willReturn(aSlow503WithBody(responseSpeed)));

    final URI uri = UriBuilder.fromPath(testUrl).scheme("http").host("localhost").port(-1).build();
  ...
  public void shouldNotExhaustConnectionPoolForNonResponsiveServer() throws ExecutionException {
    ...
    stubForWireMockClassRule(
        wmClassRule1,
        get(urlMatching(testUrl))
            .willReturn(aResponse().withFixedDelay(responseSpeed).withFault(Fault.EMPTY_RESPONSE)));
    stubForWireMockClassRule(
        wmClassRule2,
        get(urlMatching(testUrl))
            .willReturn(aResponse().withFixedDelay(responseSpeed).withFault(Fault.EMPTY_RESPONSE)));
  ...
  private void verifyBehaviourForClassRule(
      WireMockClassRule wireMockClassRule, RequestPatternBuilder requestPatternBuilder) {
    WireMock.configureFor("localhost", wireMockClassRule.port());
    WireMock.verify(requestPatternBuilder);
  }

  private void verifyNeverCalledForClassRule(
      WireMockClassRule wireMockClassRule, RequestPatternBuilder requestPatternBuilder) {
    WireMock.configureFor("localhost", wireMockClassRule.port());
    WireMock.verify(0, requestPatternBuilder);
  }

  private void stubForWireMockClassRule(
      WireMockClassRule classRule, MappingBuilder mappingBuilder) {
