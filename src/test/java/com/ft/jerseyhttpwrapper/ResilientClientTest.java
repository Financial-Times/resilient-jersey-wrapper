...
public class ResilientClientTest {
    ...
    @Rule
    public WireMockRule wm2 = new WireMockRule(wireMockConfig().port(0));

    @Rule
    public WireMockRule delayedWm = new WireMockRule(wireMockConfig().port(0));
    ...
    public void init() throws Exception {
        ...
        builder = ResilientClientBuilder.inTesting(wmNode)
    ...
    public void supportsSingleValuedRequestHeaders() {
        ...
        wm.verify(getRequestedFor(urlEqualTo("/something")).withHeader("X-Request-Header", equalTo("request-value")));
    }
    ...
    public void supportsMultiValuedRequestHeaders() {
        ...
        resource(builder.build()).header("X-Multi-Request-Header", "request-value-1")
    ...
    public void shouldRetryForRecoverableStatus() {
        ...
        Client client = ResilientClientBuilder.in(DummyClientEnvironment.inTesting())
                .using(endpointConfiguration)
                .withPrimary(new SimpleHostAndPortProvider(wmNode1, wmNode2)) 
                .withSecondary(new NullHostAndPortProvider())
                .build();

        wm.stubFor(post(urlEqualTo("/firstRequestFails"))
                .willReturn(aResponse().withStatus(500)));
                
        wm2.stubFor(post(urlEqualTo("/firstRequestFails"))
                .willReturn(aResponse().withStatus(200)));
        
        ClientResponse response = client.resource("http://localhost:" + wm.port() + "/firstRequestFails").post(ClientResponse.class);
    ...
    public void shouldNotRetryNonIdempotentMethodByDefaultWhenRemoteStateUncertain() {
        ...
        assertThat("Expecting ClientHandlerException", caught, instanceOf(ClientHandlerException.class));
        assertThat("Expecting SocketTimeoutException", caught.getCause(), instanceOf(SocketTimeoutException.class));
    ...
    public void shouldRetryNonIdempotentMethodWhenRemoteStateUncertainIfConfigured() {
        ...
        final ClientResponse response = client.resource("http://localhost:" + delayedWm.port() + "/timeout/2").post(ClientResponse.class);
    ...
    public void shouldRetryNonIdempotentMethodWhenRemoteStateCertain() throws Exception {
        ...
        final ClientResponse response = client.resource("http://localhost:7/timeout/3").post(ClientResponse.class);
    ...
    public void shouldHonourLargeTimeOuts() {
        ...
        delayedWm.stubFor(post(urlEqualTo("/timeout/4")).willReturn(aResponse().withStatus(200).withFixedDelay(14500)));
        wm.stubFor(post(urlEqualTo("/timeout/4")).willReturn(aResponse().withStatus(200)));

        final ClientResponse response = client.resource("http://localhost:" + delayedWm.port() + "/timeout/4").post(ClientResponse.class);
    ...
    private void testSupportFor(RequestMethod method, String body) {
        ...
        List<LoggedRequest> requests = wm.findAll(new RequestPatternBuilder(ANY, urlEqualTo("/something")));
    ...
    private WebResource resource(Client client) {
        ...
        URI someUri = UriBuilder.fromPath("/something").scheme("http").port(wm.port()).host("localhost").build();
