...
public class RandomHostAndPortProviderTest {
  ...
  @ClassRule public static WireMockClassRule wmClassRule5 = new WireMockClassRule(0);

  @Rule public WireMockClassRule instanceRule5 = wmClassRule5;
  ...
  public void shouldTryAllNodesInKnownSequence() {
    ...
    ResilientClient resilientClient =
        ResilientClientBuilder.inTesting(mockHost(wmClassRule1))
  ...
  public void shouldStopFailingOverAfterTryingEachHost() {
    ...
    for (WireMockClassRule mockHost : wireMockServices()) {
      stubForWireMockClassRule(
          mockHost,
          WireMock.get(WireMock.urlMatching("/testing"))
    ...
    for (WireMockClassRule mockHost : wireMockServices()) {
      ...
      requests.addAll(WireMock.findAll(WireMock.getRequestedFor(urlMatching("/testing"))));
    }
  ...
  private URI someUrl() {
    return UriBuilder.fromPath("/testing").scheme("http").host("localhost").build();
  }
