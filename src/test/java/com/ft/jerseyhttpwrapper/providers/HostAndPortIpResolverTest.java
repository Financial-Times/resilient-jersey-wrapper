...
public class HostAndPortIpResolverTest {
	...
	public void should_resolve_one_hostport_with_one_ip_to_one_hostport() throws Exception {
		...
		final List<HostAndPort> hostAndPorts = hostAndPortIpResolver.resolveAll(Arrays.asList(HostAndPort.fromString("host1:80")));
	...
	@Test
	public void should_resolve_one_hostport_with_two_ips_to_two_hostsports_with_different_ips_but_same_port() throws Exception {
	...
	@Test
	public void should_resolve_two_hostports_with_same_single_ip_but_different_ports_to_two_hostports_same_ip_but_different_ports() throws Exception {
		when(hostToIpMapper.mapToIps("host1")).thenReturn(new InetAddress[] {host1Ip});

		final HostAndPortIpResolver hostAndPortIpResolver = new HostAndPortIpResolver(hostToIpMapper);

		final List<HostAndPort> hostAndPorts = hostAndPortIpResolver.resolveAll(
	...
	@Test
	public void should_resolve_two_hostports_with_different_hosts_with_multiple_ips_and_same_ports() throws Exception {
	...
	@Test
	public void should_resolve_two_hostports_with_different_hosts_and_ports_with_multiple_ips_and_expected_ports() throws Exception {
		when(hostToIpMapper.mapToIps("host1")).thenReturn(new InetAddress[] {host1Ip, host3Ip});
		when(hostToIpMapper.mapToIps("host2")).thenReturn(new InetAddress[] {host2Ip, host4Ip});
	...
	public void should_deduplicate_hosts_that_resolve_to_same_ips() throws Exception {
		...
		final List<HostAndPort> hostAndPorts = hostAndPortIpResolver.resolveAll(
				Arrays.asList(HostAndPort.fromString("host1:80"), HostAndPort.fromString("host2:81"), HostAndPort.fromString("host2:80"))
		);
