...
public class RandomIpHostAndPortProviderTest {
	...
	public void should_resolve_single_host_with_two_ips_to_two_host_and_port() throws Exception {
		...
		List<HostAndPort> hostAndPortNames = Arrays.asList(
				HostAndPort.fromString("host:80")
		);
		...
		assertThat(hostAndPortProvider.getHostNames(), hasItems(hostAndPortIps.get(0), hostAndPortIps.get(1)));
	}
	...
	public void should_resolve_list_of_hosts() throws Exception {
		...
		assertThat(hostAndPortProvider.getHostNames(), equalTo(hostAndPortIps));
		assertThat(hostAndPortProvider.getHostNames(), hasItems(hostAndPortIps.get(0), hostAndPortIps.get(1), hostAndPortIps.get(2)));
	}
