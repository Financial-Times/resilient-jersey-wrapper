...
public class DynamicDNSProviderTest {
    ...
    @Mock
    HostAndPortIpResolver ipResolver;

    @Rule
    public WireMockRule wm1 = new WireMockRule(wireMockConfig().port(0));

    @Rule
    public WireMockRule wm2 = new WireMockRule(wireMockConfig().port(0));

    @Rule
    public WireMockRule wm3 = new WireMockRule(wireMockConfig().port(0));

    @Rule
    public WireMockRule wm4 = new WireMockRule(wireMockConfig().port(0));
    ...
    public void setIpResolver() {
        ...
        when(ipResolver.resolve(hostA)).thenReturn(Arrays.<HostAndPort>asList(wiremockPort(wm1), wiremockPort(wm2)));
        when(ipResolver.resolve(hostB)).thenReturn(Arrays.<HostAndPort>asList(wiremockPort(wm3), wiremockPort(wm4)));

        client = ResilientClientBuilder.inTesting().named("test").usingDNS().withHostAndPortResolver(ipResolver).build();
    }
    ...
    public void shouldBalanceOverMultipleHostnamesAndIPs() {
        ...
        // and hosts 3 and 4 had the same behaviour as 1 and 2 did earlier: failing once then trying again and winning
        wm3.verify(1, ourHit());
        wm4.verify(1, ourHit());
    ...
    private String hit(String hostAndPort) {
        ...
        URI hitUri = UriBuilder.fromPath(PATH).host(endpoint.getHostText()).port(endpoint.getPort()).scheme("http").build();
