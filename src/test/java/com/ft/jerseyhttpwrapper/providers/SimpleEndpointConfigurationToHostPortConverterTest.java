...
public class SimpleEndpointConfigurationToHostPortConverterTest {
  ...
  public void should_convert_simple_config_using_main_port() throws Exception {
    final SimpleEndpointConfiguration configuration =
        new SimpleEndpointConfiguration("http", "host", 80, 8080);

    final List<HostAndPort> hostAndPorts =
        new SimpleEndpointConfigurationToHostPortConverter(false)
            .convertAll(Arrays.asList(configuration));
  ...
  public void should_convert_simple_config_using_admin_port() throws Exception {
    ...
    final List<HostAndPort> hostAndPorts =
        new SimpleEndpointConfigurationToHostPortConverter(true)
            .convertAll(Arrays.asList(configuration));
  ...
  public void should_deduplicate_hosts() throws Exception {
    ...
    assertThat(
        hostAndPorts,
        equalTo(Arrays.asList(HostAndPort.fromString("a:8080"), HostAndPort.fromString("c:8082"))));
  }
}
